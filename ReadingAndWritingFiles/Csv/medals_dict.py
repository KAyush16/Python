...
# writing data to the csv file using the DictWriter
with open("country_medals.csv","w", encoding="utf-8") as output_file:
    writer=csv.DictWriter(output_file,fieldnames=columns)
    '''csv.DictWriter(output_file, fieldnames=columns) creates a DictWriter object that will write dictionaries to the specified CSV file.
    It takes the file object (output_file) and the list of field names (columns) as arguments.'''
    # writing header
    writer.writeheader() # it forms as a header 
    #writing data rows 
    for row in medals_table:
        writer.writerow(row)
    # or use : writer.writerow(sorted(medals_table))
        
'''
# Reading the CSV file to confirm the data was written correctly
with open('country_medals.csv', mode='r', newline='') as csvfile:
